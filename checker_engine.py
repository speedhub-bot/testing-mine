...
def _clone_session(session):
    """Create a new Session with auth state copied from the original.
...
class CaptureObject:
    ...
    def builder(self) -> str:
        ...
        lines = [
            sep,
            f'  Account: {_account_label(self.email)}',
    ...
    def hits_line(self) -> str:
        ...
        parts = [_account_label(self.email)]
...
class CheckerEngine:
    def __init__(self, user_id, account_list, settings, db, bot, loop):
        self.user_id = user_id
        self.account_list = account_list[:1]
        self.threads = 1
        ...
        self.valid_mail = 0
        self.checked = 0
        self.total = len(self.account_list)
    ...
    def check_account(self, account: str) -> None:
        ...
        for auth_attempt in range(max_auth_retries):
            ...
            try:
                ...
                if token:
                    break
                # Auth failed; retry once directly in case Microsoft was transient.
                session.close()
        ...
        if not token:
            self.write_dedupe(self.results_dir, 'bad.txt', f'{_account_label(email)}\n')
            with self.lock:
                self.bad += 1
                self.checked += 1
        ...
        try:
            ...
            has_mc = mc_mod.checkmc(
                ...
                self.config, self.config.get('max_retries', 3),
                self.lock, self.results_dir,
            ...
            self.write_dedupe(self.results_dir, 'errors.txt', f'{_account_label(email)} ({e})\n')
            with self.lock:
                self.errors += 1
            self.db.update_stats(self.user_id, errors=1)
        finally:
        ...
        while self.is_running:
            ...
            with self.lock:
                if self.account_list:
                    account = self.account_list.pop(0)
                else:
                    break
            if account:
    ...
    def start(self) -> None:
        thread_count = min(self.threads, max(1, len(self.account_list)), 1)
        threads_list = []
